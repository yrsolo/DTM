...
from src.infra.yc_monitoring import build_metric_point, normalize_metric_labels
from src.observability.metrics import YandexMonitoringMetricsClient
...
class YandexMonitoringMetricsClientTestCase(unittest.TestCase):
    ...
    def test_client_writes_metric_using_monitoring_adapter(self) -> None:
        ...
        with patch("src.observability.metrics.write_metrics", side_effect=_fake_write_metrics):
    ...
    def test_client_swallow_write_failure_and_logs_warning(self) -> None:
        logger = _RecordingLogger()
        with patch("src.observability.metrics.write_metrics", side_effect=RuntimeError("boom")):
            client = YandexMonitoringMetricsClient(
                folder_id="folder-1",
                iam_token_provider=lambda: "iam-test",
                logger=logger,
        ...
        self.assertEqual(event, "monitoring_metric_emit_failed")
        self.assertEqual(fields["metric"], "dtm.render.cells_written")
