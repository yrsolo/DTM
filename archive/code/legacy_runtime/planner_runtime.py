"""DEPRECATED: reference-only legacy planner orchestration facade."""
...
from archive.code.legacy_runtime.core.bootstrap import PlannerDependencies, build_planner_dependencies
...
class _SheetInfoView:
    ...
    @classmethod
    def from_mapping(cls, data: Mapping[str, Any]) -> "_SheetInfoView":
        return cls(
            spreadsheet_name=str(data.get("spreadsheet_name", "")),
            sheet_names=dict(data.get("sheet_names", {})),
        )
...
class GoogleSheetPlanner:
    ...
    def __init__(
        ...
        self.sheet_info = _SheetInfoView.from_mapping(sheet_info_data)
        ...
        source_sheet_info = getattr(getattr(dependencies, "task_repository", None), "source_sheet_info", None)
        if source_sheet_info is not None:
            self.source_sheet_info = source_sheet_info
        else:
            self.source_sheet_info = _SheetInfoView.from_mapping(sheet_info_data)
    ...
    def designer_task_to_calendar(
        ...
        query_context = build_task_query_context(self.task_repository.get_all_tasks())
