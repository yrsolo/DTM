...
- `TimerPipeline.run(RunRequest(...))` invokes `SnapshotUpdateApi.update(...)` via `src/contexts/snapshot/module.py`
...
## 3) Query runtime
- browser-read entry: `src/contexts/access_api/application/browser_read_api.py` via `src/entrypoints/http/router.py`
- task-list adapter: `src/contexts/access_api/internal/primary_task_list_read_api.py`
- task-list orchestration: `src/contexts/access_api/application/primary_task_list_read_service.py`
