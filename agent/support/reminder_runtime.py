...
from src.platform.contracts.adapters import ChatAdapter, LoggerAdapter, MessageAdapter, NullLogger
from src.platform.policies.reminder_policy import classify_delivery_error
from src.core.task_query_adapter import build_task_query_context, query_projections
...
class Reminder:
    ...
    def day_messages(self, tasks: Iterable[Any], day: pd.Timestamp) -> list[str]:
        ...
        for task in tasks:
            ...
            stages = _filter_stages(task.timing[day])
            if stages:
                stages = ', '.join(stages)
                day_lines.append(
...
_REMINDER_CFG = load_config()
_HIDDEN_STAGE_NAMES = list(_REMINDER_CFG.mapping.hidden_stage_names)


def _filter_stages(stages: Iterable[str], hidden_words: Iterable[str] | None = None) -> list[str]:
    words = [str(word).lower() for word in (hidden_words if hidden_words is not None else _HIDDEN_STAGE_NAMES)]
    return [
        stage for stage in stages
        if not any(word in str(stage).lower() for word in words)
    ]
