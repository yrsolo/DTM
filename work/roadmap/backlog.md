...
- Current active structural cleanup is top-level `src` simplification: dead historical roots are gone, `entrypoints_adapters` has been folded into `access_api`, browser masking has left `services/access`, dead service-era shelves in `services/{notify,render,mappers,sync}` are gone from tracked code, and the next decision is whether the remaining `services` core is justified or the next competing-center cut.
