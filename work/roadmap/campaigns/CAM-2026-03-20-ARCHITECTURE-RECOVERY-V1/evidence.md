...
- 2026-03-20: removed the old HTTP access-api wrapper modules (`frontend_compat_handlers`, `frontend_v2_handler`, `info_handler`, `people_snapshot_handler`, `task_attachment_read_handler`, `frontend_response_cache`); the active read path now enters `access_api` only through `src/entrypoints/http/router.py` and context-owned internals.
- 2026-03-20: removed `src.entrypoints.index_dispatcher.py`; active top-path ownership now reads directly through `index.py`, `src/entrypoint/handler.py`, and the thin shells.
