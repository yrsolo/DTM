...
| Top-path clarity | `entrypoint` + `platform.runtime` | `index.py -> handle_entrypoint(... get_http_shell/get_worker_shell/get_trigger_shell, telegram_webhook_path=_get_app_context()...)` | `index.py -> entrypoint handler -> runtime shell/context` | partial | `_get_app_context()` seam and early webhook-path fetch keep a small but real top-path indirection | low | remove early app-context lookup for webhook path and keep entrypoint on pure delegation inputs | top-path readability guard |
