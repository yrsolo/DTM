...
- CAM-UNIFIED-API-INGRESS-V1 P01: switch repo/operator URLs to path-based `/test-front` and `/prod` base paths and add unified API gateway rollout script
- CAM-UNIFIED-API-INGRESS-V1 P02: create unified Yandex API Gateway on `dtm.solofarm.ru` for `/test-front/*` and `/prod/*`, replace Yandex DNS `dtm.solofarm.ru` CNAME to the new gateway, and keep old `dtm-api-*` domains as rollback paths during propagation
