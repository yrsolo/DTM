...
- CAM-2026-03-21-REPO-HYGIENE-POLISH-V1: repo hygiene pass is delivered; active docs/scripts are aligned, stale local shells are removed, `agent/` now has a local map, and the next blocker is architectural rather than hygienic: active code still splits domain reality between the root `core/` package and `src/core/`.

## Done

- CAM-2026-03-21-SRC-TOPLEVEL-CLEANUP-V1 completed: the active `src` map now reads as `config / contexts / core / entrypoints / platform`, while old technical roots were redistributed or removed and archive material moved into the top-level `archive/` home.
...
- Current repo-hygiene follow-up blocker is no longer stale references; it is the dual active domain-root story between `core/` and `src/core/`.
