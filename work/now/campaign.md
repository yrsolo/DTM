...
- CAM-2026-03-21-SNAPSHOT-MODULE-SURFACE-V1 is still in progress, but it has crossed one more stage: active consumers now read `snapshot` through `module/api` instead of `public/capability`; the remaining work is no longer seam cleanup and has become a deeper API-contract redesign question.
