...
- Current focus inside `CAM-2026-03-21-SRC-TOPLEVEL-CLEANUP-V1`: dead roots are removed, `entrypoints_adapters` is gone, browser masking is folded into `access_api`, dead service-era shelves and leftover files in `services/{notify,render,mappers,sync,pipeline_runtime,readmodel_builder,source_policy,sync_service}` are gone from tracked code, and the next architectural decision is whether the remaining `services` core is justified or the next competing-center cut.
