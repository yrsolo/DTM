...
| agile/sprint_current.md | 2026-02-27 | TeamLead | verified board sections and WIP=1 discipline; synchronized DTM-10 lifecycle (`V rabote` -> `Gotovo`) | high | local sprint board is current for this sprint cycle |
...
| doc/03_reconstruction_backlog.md | 2026-02-27 | TeamLead | Stage 0 items 0.3, 0.4, 0.5 remain aligned; Stage 1 status extended with DTM-10 people-contract increment | high | backlog status now reflects DTM-8/9/10 Stage 1 progress |
| doc/02_current_modules_and_functionality.md | 2026-02-27 | TeamLead | synchronized with repository changes for task + people contract hardening (`core/repository.py`, `core/people.py`) | high | module doc covers latest Stage 1 hardening details |
...
| Jira control plane | 2026-02-27 | TeamLead | `.env` contains required `JIRA_*` keys; REST check `/rest/api/3/myself` returned `200`; DTM-10 lifecycle/comment updates succeeded via API | high | control plane is available when runtime loads `.env` values |
...
4. Evidence used in this iteration:
   ...
   - `.venv\Scripts\python.exe local_run.py --mode reminders-only --dry-run` fails in pre-existing reminder runtime path (`httpx.AsyncClient(... proxies=...)`), tracked as follow-up increment.
