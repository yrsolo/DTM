...
class AttachmentMetadataStore:
    ...
    def _replace_task_item(snapshot: ExtraSnapshot, *, task_id: str, attachments: list[AttachmentMeta], now: datetime) -> ExtraSnapshot:
        ...
        return ExtraSnapshot(
            version=max(int(snapshot.version or 2), 1),
            updated_at_utc=now,
            items_by_task_id=items,
        )
    ...
    def _sort(items: list[AttachmentMeta]) -> list[AttachmentMeta]:
        return sorted(
            ...
            key=lambda item: (
                ...
                getattr(item, "uploaded_at_utc", datetime.now(timezone.utc)).isoformat(),
                str(getattr(item, "attachment_id", getattr(item, "id", "")) or ""),
            ),
        )
    ...
    def create_pending(
        ...
        *,
        ...
        record = AttachmentMeta(
            ...
            filename_original=str(filename or "").strip(),
            filename_display=str(filename or "").strip(),
    ...
    def mark_uploaded_unverified(
        ...
        *,
        ...
        return self._update_status(
            ...
            status=ATTACHMENT_STATUS_UPLOADED_UNVERIFIED,
            snapshot_visible=False,
    ...
    def mark_delete_pending(self, *, task_id: str, attachment_id: str, deleted_by_user_id: str) -> AttachmentMeta:
        return self._update_status(
            task_id=task_id,
            attachment_id=attachment_id,
            status=ATTACHMENT_STATUS_DELETE_PENDING,
            snapshot_visible=False,
            deleted_by_user_id=str(deleted_by_user_id or "").strip(),
    ...
    def mark_deleted(self, *, task_id: str, attachment_id: str, deleted_by_user_id: str, warning: str = "") -> AttachmentMeta:
        ...
        return self._update_status(
            task_id=task_id,
            attachment_id=attachment_id,
            status=ATTACHMENT_STATUS_DELETED,
            snapshot_visible=False,
            deleted_by_user_id=str(deleted_by_user_id or "").strip(),
            deleted_at_utc=datetime.now(timezone.utc),
            warnings=warnings,
