...
from src.services.attachments.policy import canonicalize_attachment_mime
    ...
    def finalize(self, *, task_id: str, attachment_id: str) -> VerifiedAttachmentObject:
        ...
        if actual_size != int(record.size_bytes):
            self._metadata_store.mark_failed(
                ...
                error_code="attachment_size_mismatch",
                error_message="Uploaded object size does not match requested size.",
            )
        ...
        if actual_mime != canonicalize_attachment_mime(record.mime_type):
            self._metadata_store.mark_failed(
                task_id=record.task_id,
                attachment_id=record.attachment_id,
                error_code="attachment_mime_mismatch",
                error_message="Uploaded object mime type does not match requested mime type.",
            )
        ...
        self._metadata_store.mark_uploaded_unverified(
            task_id=record.task_id,
            attachment_id=record.attachment_id,
            storage_etag=storage_etag,
            storage_version=storage_version,
        )
        return VerifiedAttachmentObject(
