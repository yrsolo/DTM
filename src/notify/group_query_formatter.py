...
from .model import ReminderGroup
from .usecase import normalize_person_name
...
class GroupQueryFormatter:
    ...
    def build_tasks_reply(
        ...
        *,
        ...
        for day, tasks in ((today, list(selected.tasks_today or [])), (next_workday, list(selected.tasks_next_workday or []))):
            for task in tasks:
                lines.append(
                    f"{idx}. {day.strftime('%d.%m')} - {task.sheet.title} ({self._stage_preview(task, day)})"
                )
                idx += 1
        return "\n".join(lines)
    ...
    def build_deadlines_reply(self, groups: list[ReminderGroup], *, today: date, next_workday: date) -> str:
        ...
        for idx, (day, task, owner_name) in enumerate(preview, start=1):
            lines.append(
                f"{idx}. {day.strftime('%d.%m')} - {task.sheet.title} [{owner_name}] ({self._stage_preview(task, day)})"
            )
