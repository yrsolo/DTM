...
class ReminderJob:
    ...
    async def _enhance_one(self, draft_text: str, semaphore: asyncio.Semaphore) -> tuple[str, str]:
        ...
        async with semaphore:
            try:
                enhanced = await self._enhancer.chat(
                    messages=[
                        {"role": "system", "content": self._helper_character},
    ...
    async def _send_with_retry(self, *, chat_id: str, message: str, owner_name: str) -> tuple[bool, dict[str, int]]:
        counters = {
            "send_errors": 0,
    ...
    async def run(self, req: ReminderRequest) -> ReminderResult:
        ...
        result = ReminderResult(
            ...
            delivery_counters={
                ...
                "send_error_permanent": 0,
                "send_error_unknown": 0,
                "fallback_default_chat": 0,
            },
            enhancement_counters={
                "provider": "mock" if isinstance(self._enhancer, _MockEnhancer) else "llm",
                "candidates_total": len(groups),
                "attempted": 0,
                "succeeded": 0,
                "fallback_empty": 0,
