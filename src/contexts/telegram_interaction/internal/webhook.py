...
class TelegramWebhookHandler:
    def __init__(self, ctx, parser: TelegramUpdateParser | None = None, command_router: TelegramCommandRouter | None = None) -> None:
        self._ctx = ctx
    ...
    def handle(self, req: HttpRequest):
        ...
        if secret_required and (not expected_secret or actual_secret != expected_secret):
            if metrics is not None:
                metrics.counter("dtm.telegram.rejected_total", labels={"env": env_name, "module": "telegram", "operation": "webhook", "result": "forbidden"})
        ...
        with timed(metrics, "dtm.telegram.enqueue_ms", {"env": env_name, "module": "telegram", "operation": "webhook", "result": "accepted"}):
            ...
            if parsed is None:
                if metrics is not None:
                    metrics.counter("dtm.telegram.rejected_total", labels={"env": env_name, "module": "telegram", "operation": "parse", "result": "unsupported_update"})
                return json_response(200, {"artifact": "telegram_webhook", "status": "ignored", "reason": "unsupported_update"})
            routed = self._command_router.route(parsed, bot_username=str(self._ctx.deps.get("tg_bot_username", "")), default_chat_id=str(self._ctx.deps.get("default_chat_id", "")))
            if routed is None:
                if metrics is not None:
        ...
        if metrics is not None:
            ...
            metrics.counter("dtm.telegram.accepted_total", labels={"env": env_name, "module": "telegram", "operation": "webhook", "result": "accepted"})
        if logger is not None:
