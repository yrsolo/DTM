...
class DesignersRenderUseCase:
    ...
    def _select_tasks(self, req: RenderRequest) -> list[_DesignerTask]:
        ...
        result.sort(key=lambda item: (_next_due(item.task, self._timezone_name) is None, _next_due(item.task, self._timezone_name) or date.max, str(item.task.sheet.title or "")))
        return result
    ...
    def build_plan(self, req: RenderRequest) -> RenderPlan:
        ...
        for owner in sorted(owners):
            ...
            if not owner_tasks:
                continue
            values.append(RenderCell(row=start_row, col=col, value=owner, color="#9F65CC", text_color="#FFFFFF", bold=True, font_size=12))
            for task in owner_tasks:
                note = f"{task.sheet.customer}\n{task.sheet.raw_timing}"
                values.append(RenderCell(row=row, col=col, value=str(task.sheet.title or ""), note=note))
                row += 1
            col += 1
            row = start_row + 1
        values.append(RenderCell(row=1, col=1, value=format_sheet_timestamp(now_in_timezone(self._timezone_name))))
