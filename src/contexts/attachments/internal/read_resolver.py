...
from .contracts import ATTACHMENT_KIND_DOC
from .policy import attachment_read_allowed
    ...
    def resolve(self, *, attachment_id: str, access, download: bool) -> AttachmentReadResult:  # noqa: ANN001
        ...
        if kind == ATTACHMENT_KIND_DOC and not download:
            ...
            if preview_state == "ready" and derived_preview_ref:
                return AttachmentReadResult(url=self._storage.generate_read_url(key=derived_preview_ref, filename="preview.pdf", download=False), filename="preview.pdf", download=False)
        ...
        return AttachmentReadResult(url=self._storage.generate_read_url(key=record.storage_key, filename=record.filename_display, download=bool(download)), filename=record.filename_display, download=bool(download))
