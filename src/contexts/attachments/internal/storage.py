...
class AttachmentStorage:
    ...
    def generate_upload_contract(self, *, key: str, mime_type: str, sign_content_type: bool = True, expires_in_seconds: int = 900) -> dict[str, Any]:
        client = self._client()
        ...
        try:
            upload_url = client.generate_presigned_url("put_object", Params=params, ExpiresIn=int(expires_in_seconds), HttpMethod="PUT")
        except Exception as error:
            raise TransientError(str(error), code="attachment_upload_contract_failed") from error
        return {"method": "PUT", "uploadUrl": upload_url, "headers": headers, "expiresIn": int(expires_in_seconds)}

    def generate_preview_upload_contract(self, *, key: str, mime_type: str = "application/pdf", expires_in_seconds: int = 900) -> dict[str, Any]:
        return self.generate_upload_contract(key=key, mime_type=mime_type, sign_content_type=False, expires_in_seconds=expires_in_seconds)
    ...
    def generate_read_url(self, *, key: str, filename: str, download: bool, expires_in_seconds: int = 300) -> str:
        client = self._client()
        disposition = "attachment" if download else "inline"
        safe_name = _content_disposition_filename(filename)
        try:
            return str(client.generate_presigned_url("get_object", Params={"Bucket": self.bucket, "Key": key, "ResponseContentDisposition": f'{disposition}; filename="{safe_name}"'}, ExpiresIn=int(expires_in_seconds), HttpMethod="GET"))
        except Exception as error:
