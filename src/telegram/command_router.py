...
class TelegramCommandRouter:
    """Maps parsed Telegram updates to internal queue commands."""
        ...
        *,
        ...
        if parsed_update.chat_type == "private" and parsed_update.chat_id == str(default_chat_id or "").strip():
            ...
            if command in RENDER_COMMANDS:
                return RoutedTelegramCommand(
                    command_name="render_timeline",
                    command_type=RENDER_TIMELINE_SHEET,
                    payload={"statuses": ["work", "pre_done"], "dry_run": False},
                )
        return None
