"""Guardrail checks for the thin active entrypoint contour."""
...
INDEX_ALLOWED_IMPORTS = {
    ...
    "src.platform.shell.handle_http_event",
    "src.platform.shell.handle_queue_event",
    "src.platform.shell.handle_trigger_event",
...
def main() -> int:
    ...
    for file_path in _iter_python_files():
        ...
        if file_path == ROOT / "index.py":
            ...
            if unexpected:
                violations.append("index.py has unexpected imports: " + ", ".join(unexpected))
    if violations:
        print("entrypoint_import_boundaries=failed")
        for violation in violations:
            print(violation)
        return 1
    print("entrypoint_import_boundaries=ok")
    return 0
