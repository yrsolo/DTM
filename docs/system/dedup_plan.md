...
| Sync/build handlers | entrypoints/jobs + `src/services/pipeline_runtime.py` | `src/handlers/sync.py`, `src/handlers/build_readmodels.py` in runtime contour | no imports from `main.py`/`index.py`; only handler tests use them | medium |

## Proposed removal sequence
1. Add explicit `legacy-only` marking in docs for handler/readmodels old path.
2. Stop running legacy handler tests in default smoke (or move to dedicated legacy suite).
3. Remove legacy handler path and `src/services/sync/sync_service.py` after test suite split.
