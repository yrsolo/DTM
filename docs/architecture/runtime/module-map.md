...
| `src/entrypoints/root/*` | Entrypoint | Thin root intake layer for parsed request routing | OK | Canonical intake surface. |
...
| `src/platform/runtime/commands/*` | Application | Internal command contracts and queue serialization | OK | Canonical async command boundary. |
| `src/platform/runtime/worker/*` | Application | Worker dispatcher, execution, status persistence | OK | Canonical async mutation runner. |
| `src/contexts/access_api/internal/masking.py` | Application | Access masking and boundary transforms | OK | Browser access policy lives at the boundary. |
| `src/platform/observability/*` | Support | Metrics, logging, timing, trace helpers | OK | Shared observability layer. |
| `src/config/*` | Support | Typed config schema and loader | OK | Canonical config source of truth. |
| `src/platform/bootstrap.py` | Support | Composition root and dependency assembly | OK | Bootstrap only; no business logic. |

## Active adapters

| Path | Role | Responsibility | State | Notes |
|---|---|---|---|---|
| `src/platform/integrations/google_sheets/*` | Adapter | Google Sheets fetch/update integrations | OK | Edge adapter for source data and render writes. |
| `src/platform/integrations/telegram/*` | Adapter | Telegram delivery integration | OK | Used by reminders and group-query flows. |
| `src/platform/integrations/llm/*` | Adapter | Optional LLM enhancement providers | OK | Optional edge integrations only. |
